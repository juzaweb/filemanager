  ...
  <div class="col-md-4">
    ...
    <table class="table table-compact table-striped">
      ...
      <tbody>
        @if ($scaled)
        <tr>
          <td>{{ trans('file-manager.resize-ratio') }}</td>
          <td>{{ number_format($ratio, 2) }}</td>
        </tr>
        <tr>
          <td>{{ trans('file-manager.resize-scaled') }}</td>
          <td>
            {{ trans('file-manager.resize-true') }}
          </td>
        </tr>
        @endif
        <tr>
          <td>{{ trans('file-manager.resize-old-height') }}</td>
          <td>{{ $original_height }}px</td>
        </tr>
        <tr>
          <td>{{ trans('file-manager.resize-old-width') }}</td>
          <td>{{ $original_width }}px</td>
        </tr>
        <tr>
          <td>{{ trans('file-manager.resize-new-height') }}</td>
          <td><span id="height_display"></span></td>
        </tr>
        <tr>
          <td>{{ trans('file-manager.resize-new-width') }}</td>
          <td><span id="width_display"></span></td>
        </tr>
      </tbody>
    </table>

    <button class="btn btn-primary" onclick="doResize()">{{ trans('file-manager.btn-resize') }}</button>
    <button class="btn btn-info" onclick="loadItems()">{{ trans('file-manager.btn-cancel') }}</button>
